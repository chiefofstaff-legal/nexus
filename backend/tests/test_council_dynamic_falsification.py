...
class _FakeStore:
    ...
    def append(self, idr: IntentDecisionRecord, user_id: str) -> dict:
        # Mirror IDRStore.append's DRY tenant_id injection so the fake
        # stays behaviourally faithful to the real store.
...
async def test_council_threads_dynamic_criterion_into_idr():
    ...
    result = await council.deliberate(_query(), user_id="test-tenant")
...
async def test_council_falls_back_when_no_vote_supplies_falsification():
    ...
    await council.deliberate(_query(), user_id="test-tenant")
