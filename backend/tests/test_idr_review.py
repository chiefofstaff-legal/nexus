...
def test_effective_status_uses_review_verdict(tmp_path: Path):
    store = IDRStore(tmp_path)
    signed_target = store.append(_make_target("sha256:doc2"))
...
def test_effective_status_latest_wins_on_multiple_reviews(tmp_path: Path):
    store = IDRStore(tmp_path)
    signed_target = store.append(_make_target("sha256:doc3"))
    target_seq = signed_target["sequence"]
...
def test_find_reviews_for_only_returns_matching_targets(tmp_path: Path):
    store = IDRStore(tmp_path)
    a_seq = store.append(_make_target("sha256:a"))["sequence"]
    b_seq = store.append(_make_target("sha256:b"))["sequence"]

    store.append(_make_review(a_seq, "confirmed", "rev-a"))
    store.append(_make_review(b_seq, "refuted", "rev-b", reviewer_label="public"))
    store.append(_make_review(a_seq, "inconclusive", "rev-c"))
...
def test_chain_still_verifies_after_reviews(tmp_path: Path):
    store = IDRStore(tmp_path)
    for i in range(3):
        store.append(_make_target(f"sha256:doc{i}"))
    store.append(_make_review(1, "confirmed", "rev-1"))
    store.append(_make_review(2, "refuted", "rev-2", reviewer_label="public"))
